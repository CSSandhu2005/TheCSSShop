...
import { MainSection } from "./lets-work-section";
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
          ...
          vec3 baseColor = vec3(1.0); // White Color For Lightning Is From Here ( Hard Coded It )
...
export const HeroSection: React.FC = () => {
  ...
  const [lightningHue, setLightningHue] = useState(195); // Default hue
  ...
  return (
    <div className="relative w-full h-screen bg-black text-white overflow-hidden ">
      ...
      {/* <HeroSectionMain /> ---> TailArk*/}
      {/* Main container with space for content */}
      <div className="relative z-20 max-w-7xl mx-auto px-4 py-52 sm:px-6 lg:px-8 h-screen lg:py-52 ">
        {/* Navigation */}
        ...
        >
          <motion.div variants={itemVariants}>
            <FeatureItem
              name="React"
              value="for base"
              position="left-0 -top-82 lg:top-82 lg:left-0"
            />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem
              name="Tailwind"
              value="for styles"
              position="left-0 -top-52 lg:top-42 lg:left-1/4 "
            />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem
              name="Framer"
              value="for web-fxs"
              position="right-0 -top-52 lg:top-42 lg:right-1/4"
            />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem
              name="Shaders"
              value="for 3D-fxs"
              position="right-0 -top-82 lg:top-82 lg:right-0 "
            />
          </motion.div>
        </motion.div>
        ...
        >
          ...
          <motion.h1
            variants={itemVariants}
            className="text-5xl md:text-7xl font-semibold mb-2"
          >
            The CSS Shop
          </motion.h1>
          <motion.h2
            variants={itemVariants}
            className="text-3xl md:text-5xl pb-3 font-light bg-linear-to-r from-gray-100 via-gray-200 to-gray-300 bg-clip-text text-transparent"
          >
            Frontend That Feels Premium
          </motion.h2>
          {/* Description */}
          <motion.p
            variants={itemVariants}
          ...
          >
            Premium frontends crafted with precision and intent. Design and code
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="mt-25 sm:mt-25 px-8 py-3 bg-white/10 backdrop-blur-sm rounded-full hover:bg-white/20 transition-colors"
          >
            Minimalistic &Premium Web-Experience
          </motion.button>
        </motion.div>
      </div>
      ...
      >
        ...
        {/* Glowing circle */}
        <div className="absolute top-[55%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-200 h-200 rounded-full bg-linear-to-b from-blue-500/20 to-purple-600/10 blur-3xl"></div>
        ...
        {/* Planet/sphere */}
        <div
          className="z-10 absolute top-[55%] left-1/2 transform -translate-x-1/2 w-150 h-150 backdrop-blur-3xl rounded-full 
bg-[radial-gradient(circle_at_25%_90%,#ffffff_15%,#000000de_70%,#000000ed_100%)]"
        ></div>
      </motion.div>
    </div>
  );
